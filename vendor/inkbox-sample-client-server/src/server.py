...
async def webhook(request: Request) -> JSONResponse | PlainTextResponse:
    ...
    # Optional ConNET integration (see network/run_inkbox_gateway.py).
    _connet_flag = os.getenv("CONNET_WEBHOOK_INTEGRATION", "").strip().lower()
    if _connet_flag in {"1", "true", "yes", "on"}:
        try:
            import connet_webhook_hook as _connet_wh
        except ImportError as exc:
            logger.warning(
                "CONNET_WEBHOOK_INTEGRATION is set but connet_webhook_hook is not importable: %s",
                exc,
            )
        else:
            try:
                await _connet_wh.on_inkbox_webhook(payload)
            except Exception:
                logger.exception("ConNET webhook hook failed")
